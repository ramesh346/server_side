...
// Define relative path for saving screenshots
const screenshotsDir = path.join(baseDir, 'images');
...
function sendMessageToChannel() {
    ...
    const message = `Start time: ${currentTime}`;
...
async function scrapeProductPage(page, url, title) {
    ...
    try {
        ...
        // Take screenshot
        const screenshotPath = path.join(screenshotsDir, `${title}.png`);
